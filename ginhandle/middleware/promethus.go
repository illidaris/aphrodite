	...
	if p.listenAddress != "" {
		go func() {
			if err := p.router.Run(p.listenAddress); err != nil {
				log.Error(fmt.Sprintf("p.router.Run failed %v", err))
			}
		}()
	}
}
...
func (p *Prometheus) getMetrics() []byte {
	...
	if err != nil {
		log.Error(fmt.Sprintf("p.Ppg.MetricsURL failed %v", err))
		return []byte{}
	}

	defer func() {
		if err := response.Body.Close(); err != nil {
			log.Error(fmt.Sprintf("response.Body.Close failed %v", err))
		}
	}()

	body, err := io.ReadAll(response.Body)
	if err != nil {
		log.Error(fmt.Sprintf("io.ReadAll failed %v", err))
		return nil
	}
...
func (p *Prometheus) sendMetricsToPushGateway(metrics []byte) {
	...
	if err != nil {
		log.Error(fmt.Sprintf("Error creating push gateway request for URL: %s %v", p.getPushGatewayURL(), err))
		return
	}
	client := &http.Client{}
	...
	defer func() {
		if err := resp.Body.Close(); err != nil {
			log.Error(fmt.Sprintf("Error closing push gateway response body %v", err))
		}
	}()
}
...
func (p *Prometheus) registerMetrics(subsystem string) {
	...
	for _, metricDef := range p.MetricsList {
		...
		if err := prometheus.Register(metric); err != nil {
			log.Error(fmt.Sprintf("%s could not be registered in Prometheus %v", metricDef.Name, err))
		}
		switch metricDef.ID {
		case "reqCnt":
...
func (p *Prometheus) computeApproximateRequestSize(r *http.Request) int {
	...
	if err != nil {
		log.Error(fmt.Sprintf("cannot read request body for size calculation %v", err))
		...
		if closeErr := r.Body.Close(); closeErr != nil {
			log.Error(fmt.Sprintf("cannot close request body after read error %v", closeErr))
		}
		return s
	}
	...
	if err := r.Body.Close(); err != nil {
		log.Error(fmt.Sprintf("cannot close request body after reading for size calculation %v", err))
	}
